...
The dogfood project did **not** reach a green full verification, and that is reported rather than papered over. What the newly-visible 21 red nodes turned out to be is itself the argument for the feature: 8 of them fail **only** on a viewport profile the project's own CI never runs (its scripts pin one Playwright project; CoDD's template runs every configured one), and most of the rest were verifying against fixtures the project seeds only under a CI-only flag. None of that is CoDD's to decide, and each candidate remedy narrows what gets verified — which is exactly the move a run reporting `0 SKIP / 65 total` makes impossible to do quietly.
