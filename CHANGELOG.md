...
### Fixed
- **`implementation_coverage` matches `common`-reclassified code nodes** (dogfood-reported)
  - Files matched by `common_node_patterns` are reclassified from `impl_file`/`test_file` to `kind="common"` (the C5 transitive-closure exemption), which silently removed them from the C8 matching pools: real implementations were reported as `missing_implementation` in every project using `common_node_patterns`. Non-markdown `common` nodes (`.md` is the codebase-wide doc discriminator, same principle as `dependency_freshness`) are now included in the implementation/test/config candidate pools. Markdown common *documents* still never satisfy an implementation hint.
- **`implementation_coverage` file-system fallback covers literal hints and bracketed paths** (dogfood-reported)
  - The FS fallback (for expected artifacts the scanner did not register as DAG nodes) previously ran only for glob hints, and raw `fnmatch` misread literal bracketed directory segments as character classes, so such paths could never match. Literal hints are now resolved directly against the file system (an existing file satisfies the hint — strictly PASS-increasing), and glob hints are additionally tried with the literal-bracket-escaped pattern form. The `additional_implementation` pass keeps its exact historical matching behavior (including the glob-FS suppression quirk) so existing projects see no new amber findings.
- **`ci_health` no longer collects `verification:` hook labels as commands** (dogfood-reported)
  - In the deploy hook schema `- verification: <name>` + `command:`/`url:` …, the `verification` value names the hook; ci_health collected it as a command string and reported a bogus `ci_verification_not_in_workflow` amber (e.g. a bare hook label that appears in no workflow). A string-valued `verification:`/`verifications:` entry is now treated as a label whenever the same mapping carries the actual spec (`command`/`run`/`script`/`test`/`url`); the spec keys are collected instead. A bare string `verification: <command>` without sibling spec keys keeps its historical meaning. Target `status` metadata remains uninterpreted: core defines no status semantics, and verification commands declared on inactive targets should be removed at the declaration site rather than skipped by keyword-matching free-form status text.
...
### Added
- **Opt-in `journey_scope` declaration for coverage axes/variants** (dogfood-reported)
  - `environment_coverage` used to force every `source: lexicon` axis variant onto every design-doc journey (full axis×journey cross product) with no way to express the declarer's intent, leaving permanent red `journey_not_executed_under_variant` violations for variants that legitimately cover only a subset of journeys (e.g. one variant owns the failure surfaces only reachable in a specific runtime tier while another owns tier-independent journeys).
  - `CoverageAxis` and `CoverageVariant` now accept an opt-in `journey_scope`: a list shorthand (include patterns), or a mapping with `include` and/or `exclude` journey-name patterns (`fnmatch`). `include: []` declares "applies to no journeys"; `exclude` is evaluated after `include`. A variant-level declaration overrides the axis-level one. Malformed declarations warn and are ignored (the variant is kept, falling back to the stricter full cross product).
  - `environment_coverage` honors the effective scope when expanding journeys per variant; `missing_test_for_variant` and `variant_criticality_unclear` are unaffected by scoping.
  - **Undeclared projects are bit-identical**: with no `journey_scope` anywhere, the full cross product, serialized axis payloads, and check results are unchanged (no new keys are emitted by `to_dict`).
- **Doc-to-doc dependency freshness check + reconciliation ledger** (v2.22.0 Mechanism A)
  - New DAG check `dependency_freshness`: for every doc→doc `depends_on` edge it compares the upstream document's current last commit with the last *acknowledged* reconciliation recorded in `<codd-dir>/reconciliation_ledger.json`. Propagation used to be an event whose negative outcome (an un-propagated upstream change) left no trace once the git-diff window moved on; the ledger turns reconciliation into durable state.
  - `codd propagate --commit` now writes ledger acknowledgements for every reconciled doc→upstream-doc pair, including HITL docs the human judged "no update needed".
  - Default severity is **amber** (advisory in `codd dag verify`, exit code unchanged); opt in to hard-gating with `dependency_freshness.severity: red` in codd.yaml. Opt out entirely with `enabled: false`. The check is added to the built-in `enabled_checks` defaults for the web/cli/iot/mobile project types (generic-type projects already run all registered checks); projects whose codd.yaml pins `dag.enabled_checks` must add it to that list — `codd dag verify` now prints a notice when it is missing (see below).
  - **The ledger is the primary mechanism; the no-ledger fallback is best-effort onboarding triage**, and is documented as such. The basic heuristic flags edges whose upstream was last committed after the downstream. Because a commit touching *both* documents carries no ordering signal between them (dogfooding surfaced real staleness masked by a bulk commit that touched both files), an equal-recency joint tip is now disambiguated via exclusive-commit history: if the upstream also changed on its own after the downstream's previous change, the edge is flagged `never_reconciled` (with the joint tip commit named in the detail). Repeated bulk touches can still mask drift — only an acknowledged ledger baseline closes that hole, which is why the missing baseline is always reported explicitly (edge count + triage caveat) and never silently passes.
  - Fixed: `kind="common"` is shared by frontmatter-declared common *documents* and `common_node_patterns`-matched common *code* files; non-markdown `common` nodes are now excluded from the doc-to-doc edge set (the reconciliation ledger only ever acknowledges `.md` paths, so code→code edges were unreconcilable false-positive candidates — surfaced by dogfooding).
- **`codd dag verify` notice for registered-but-unselected checks**
  - `enabled_checks` is an explicit allowlist (from the project-type defaults or a codd.yaml `dag:` override); checks shipped after the list was written silently never ran. `codd dag verify` (without `--check`) now prints a one-line notice naming the registered checks the allowlist leaves out, with the remediation (add them to `dag.enabled_checks` or remove the list to run all). In `--format json` the notice goes to stderr so the stdout JSON array stays parseable.
- **`enables` relationship + enablement coverage axes** (v2.22.0 Mechanism B)
  - Operations in `operation_flow` may declare the opt-in `enables` field: `enables: [{actor: ..., operations: [...], access_paths: [granted, direct]}]` — "this operation's outcome authorizes another actor to perform other operations" (capability exercise, not mere observation; the structural sibling of `visible_to`).
  - New `enablement_chain` coverage axis: a composite two-operation scenario — complete the enabling operation through its public trigger first, then prove the enabled actor completes the enabled operation through the granted access. The grant completion is structurally injected as a precondition and an `enablement_exercise` DoD obligation forbids direct-ownership fixture shortcuts.
  - New `access_path_variation:<path>` coverage axes: one happy-path variant per declared access path (default `granted`) plus an `access_path_variation:unrelated` negative proving an actor with no access relationship is rejected without persisting state (`access_path_isolation` / `no_forbidden_mutation` DoD obligations).
  - `capability_completeness` gains a third verb class `grant_verbs` (assign/grant/share/entitle/attach/link/allocate/delegate/authorize/permit, extendable via codd.yaml). A new advisory `[authorization_reachability]` flags grant operations whose target is consumed by other operations but that declare no `enables` wiring — the question "the target exists, but is the consuming actor's access dependency declared?" is orthogonal to the existing consumer-without-producer check, whose produce/consume classification is untouched.
  - `codd doctor` gains an `[enables_nudge]` advisory: an operation whose `visible_to`/`expected_outcomes` mention another declared actor while declaring no `enables` relationship is nudged to consider declaring one. Whole-word actor matching suppresses substring false positives; opt out with `enables_nudge.enabled: false`.
- **`depends_on_consistency` empty-input visibility**
  - When the propagation output contains no comparable `depends_on` values the result now reports `0 records compared — … consistency was not exercised` (plus a `records_compared` count). Pass/fail behaviour is unchanged; a baseline placeholder file can no longer masquerade as an exercised green.

### Compatibility
- All v2.22.0 mechanisms are opt-in or advisory. Projects without an `enables` declaration derive **exactly** the same scenario set as before (proven by a byte-identical comparison test). Projects without a reconciliation ledger see at most amber advisories in `codd dag verify`; exit codes are unchanged unless `dependency_freshness.severity: red` is explicitly configured. Adding `dependency_freshness` to the built-in web/cli/iot/mobile `enabled_checks` defaults and printing the unselected-checks notice change default `codd dag verify` *output* only — never the exit code.
...
### Notes
- 設計書: `/path/to/orchestrator/queue/reports/gunshi_357_arch_design.yaml`
...
### Dogfood results (example-app, cmd_467)
...
### Dogfood — example-app self-improvement re-run
...
- **Generality Gate**: zero project-specific literals (`osato` / `lms` / `caregiver` / `介護` / `the-app`) across the seven new lexicon directories.
...
### Fixed (surfaced by example-app cmd_446 dogfooding)
...
- **example-app dogfooding**: `unrepairable` 16 → 5 (deployment_completeness 2 → 1, verification_test_runtime 14 → 4). Remaining items target the `produces_state` chain and the Vitest matcher, tracked separately.

## [2.2.0] - 2026-05-08 — Elicit bug fixes + scope/phase filter (cmd_445)

### Fixed (P0 release blocker — surfaced by example-app cmd_442 dogfooding)
...
example-app PoC 結果: unrepairable は 2→8 件と「増加」したが、これは silent
...
example-app 実プロジェクト上で `codd verify --auto-repair --max-attempts 10` が
...
#### cmd_425_lms — example-app full pipeline 完走実証 (example-app `3262f9b`)
...
- **example-app 残 violation 3 件**: pre_existing 1 (deployment_completeness chain) +
  unrepairable 2 (Dockerfile dry-run / Vitest matcher runtime)。これらは ASK-2/ASK-5 反映の
  正常挙動 (CoDD 責任外として skip + report)。example-app baseline の他 check 項目の問題で、
  v1.35.0 候補 cmd_426 (example-app baseline cleanup) で解消予定。
...
- cmd_425_lms (example-app `3262f9b`) — full pipeline PARTIAL_SUCCESS 完走実証
...
殿哲学「機能要件 + 制約だけ書けば全自動」が実プロジェクト example-app で実証された
**記念マイルストーン release**。
...
example-app 統合実証が end-to-end レベルへ進み、CoDD は **内側 (内部整合性) + 外側 (対象環境網羅性)
の両次元 coherence** を実機で確認。

### cmd_424 — Stripe Webhook Cleanup + cmd_420 Typecheck Loop 実証 (caveat_3 完全解消)

example-app 既存 dirty Stripe webhook route の missing module imports (cmd_412b 期間中の
取りこぼし) を方針 B (route 簡素化) で解消、cmd_420 typecheck loop の実プロジェクト動作を
実証。

- **example-app 28c4144**: Stripe webhook route 簡素化 (`@/lib/api/rate-limiter` /
  `@/lib/stripe/client` / `@/modules/payments/handlers` / `@/modules/payments/types`
  欠落 import を削除し Stripe SDK 直使用)
- `npm run typecheck` PASS
- `codd implement run --enable-typecheck-loop` PASS — cmd_420 typecheck loop が example-app で
  実機動作確認

### cmd_423 — example-app LMS Integration (caveat_2 完全解消 + caveat_1 構造実証)

example-app に cdp_browser config + project CDP plugin を統合し、smartphone_se 実機 journey
...
```
codd dag run-journey login_to_dashboard --path /home/tono/example-app \
...
- **example-app vitest**: tests/e2e/environment-coverage.spec.ts **3 PASS**
- **example-app prisma:validate**: PASS
- **git diff --check**: codd-dev / example-app 両者 clean
...
- C9 修復 (attempt_0 PASS) の後、example-app 既存 baseline の `node_completeness` /
  `deployment_completeness` 失敗および Vitest matcher / no-tests 系 failure が残存し、
  次 attempt は別 proof-break (auth proof-break) に落ちた
- これは **C9 修復経路自体の問題ではなく**、example-app baseline の他 check 項目の問題

**v1.34.0 候補 (cmd_425 等)**: example-app baseline 修復 (node_completeness /
deployment_completeness / Vitest matcher 整備) で full auto-repair pipeline 完走を達成予定。
完成すれば「自律自己修復実装駆動」の最終形に到達する。

### Phase 構成と commits

- cmd_424 (example-app `28c4144`) — Stripe webhook typecheck debt fix
- cmd_423 lms (example-app `3314ae5`) — CDP browser proof integration
...
### cmd_422_lms — example-app Coverage Axes Proof
...
cmd_422 は **codd-dev 側 core 実装は完了**したが、example-app との **end-to-end 統合実証** は
3 件の caveat が残存:
...
2. **実機 CDP run-journey 未達成** —
   `codd dag run-journey login_to_dashboard --axis viewport=smartphone_se` は example-app
...
3. **cmd_420 typecheck loop 動作未確認 (example-app 側)** —
   example-app の既存 dirty Stripe webhook route が missing module import を持つため
   `npm run typecheck` 自体が失敗、cmd_420 typecheck loop 動作確認まで到達せず。
   codd-dev 側 cmd_420 実装 (v1.31.0 で release) は無回帰。

これらは **codd-dev 側 core 実装は完了済**、example-app 側 setup の問題。次 release (v1.33.0
候補 cmd_423 + cmd_424) で example-app 側設定追加 + dirty file cleanup 後、auto-repair 統合
...
- cmd_422_lms (example-app `1e8eabf`) — coverage_axes proof + smartphone fix
...
cmd_404 (v1.30.0) の standalone auto-repair が example-app 環境で詰まった問題を解消:

- Prefer standalone verify for auto-repair
- Skip missing proof-break checks with warnings
- `tests/integration/standalone_repair_skeleton/` を同梱 (example-app hardcode 回避、
...
- cmd_419: example-app hardcode ゼロ、generic standalone project skeleton で再現
...
正式化、example-app で C8 公式 PASS 確認済。
...
`codd verify --auto-repair` が codd-pro 不在環境 (example-app 等) で動作するように修正。
...
web app/mobile/next.js/react/django/rails/lms/example-app`。
...
- layer A (codd/repair/*.py): zero hit (claude/openai/cookie/oauth/nextauth/safari/iphone/
  chromium/powershell/react/smoke/headless/example-app)
...
### example-app 実証 (cmd_397_lms)

example-app に `codd.yaml [verification.templates.cdp_browser]` 追加 + cookbook plug-in
...
### example-app 実証 (HTTP × `__Secure-` 事故の構造的検出)
...
- `codd dag verify --check deployment_completeness` → example-app で **PASS**
- VPS 0.0.0.0:3000 root + /login 共に **200**、殿ログイン可能 を確認
...
example-app の「役割別ホーム未実装」(cmd_350 Phase C.5 NG #2) のような task 完了漏れも、
...
10 秒判断 → 違ったら after-fact patch。example-app screen-flow 不在事故のような「設計書
漏れ」を構造的に再発防止。
...
- cmd_350 Phase C.5 NG #2 (login 後遷移なし) の根本原因 (CoDD のノード only / エッジ非対応)
  を構造的に解消。example-app login/page.tsx の callback wiring 漏れも cmd_368 の wrapper
  rules で再発防止。
...
- Validated on example-app (large Next.js codebase): endpoint nodes 1 → 109, drift detection 186 cases. See cmd_334 report.
