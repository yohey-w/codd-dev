...
- **`codd/dag/checks/acceptance_evidence.py` — the gate**, registered, runs inside plain `codd verify`. Red: `runtime_evidence_not_executable` (a runtime obligation in a project whose `runtime_smoke` is off — the silent skip), `unbound_acceptance`, `unresolved_evidence`, `manual_evidence_missing`, `vb_registry_missing`, `stale_manual_evidence`, `param_not_referenced`. Amber: `undeclared_numeric`, `off_shipped_path`, `multiple_implementers`, `reachability_unknown`. Every severity is configurable, so a project may downgrade a class while migrating — never silence it. A project that declares no criteria reports `skip` with `checked_count == 0`, which the materiality overlay keeps distinct from a verified clean run.
- **`test_coverage.require_vb_table` (NEW, default `true`) — BEHAVIOR CHANGE.** An empty VB registry now FAILS the implement-time coverage gate for a project that has something to certify (it declares acceptance criteria, or structurally expects a canonical registry). A project that declares neither is untouched. Set `false` to restore the old notice-and-pass.
- **`codd acceptance list | sync | record` — the other half of a default-on gate.** `sync` derives one VB row per acceptance criterion into the canonical registry, deterministically, no AI: append-only, id-stable (`VB-<req-id>`), idempotent. `record` writes a MANUAL verdict into `<codd-dir>/acceptance_ledger.json` together with the content hashes of the implementation it accepted — so when that implementation changes, the verdict expires and the criterion is unaccepted again, which is the honest state. The ledger is never refreshed automatically; a self-updating ledger detects nothing.
- **(c) is a NAME check, not a number grep.** The defect is a value copied three times, and a copy cannot notice that the original moved. Grepping the criterion's digits would catch it only by luck and would fire on every date, version and paper size on the way. Declare `params: per_sheet=15` beside the criterion and read `per_sheet` in the test; a declared parameter missing from the bound evidence is red. A bare literal in a criterion that declares no parameters is amber — a nudge, not an accusation, because the check cannot know whether that number is load-bearing. The tokenizer rejects every number that is part of a larger token (`F-E2`, `A4`, `v1.7`, `2026-08-22`, paths), which is where the false positives live.
- **(b) asks its question about EVIDENCE.** Not about every file that mentions a requirement id: requirements legitimately span layers, and a migration being unreachable from a page is architecture, not a defect. The evidence for a criterion is the tests bound to it plus the tests of any file claiming its id; if none of them, nor anything they exercise, appears in the import closure of the requirement's entry point, that is `off_shipped_path`. Entry points come from the criterion's own `operation_flow.<id>` — the operation's `route:` resolved through the project's declared `filesystem_routes`, or an explicit `entry_file:` that always wins over inference. All of (b) is amber: reachability is only as good as per-language import extraction, and an entry that cannot be resolved is reported as `reachability_unknown` rather than guessed at in either direction.

Tests: 49 new (`tests/dag/test_acceptance_evidence.py`), all `tmp_path` fixtures — synthetic miniatures of the failing shape, no file copied from any real project, no framework or project literal in the check.

Dogfood — the real project, at the commit BEFORE the fix, with the requirement documents untouched (69 acceptance criteria parsed from its existing tables):

- **(a) demotion**: 50 criteria declare runtime obligations and `runtime_smoke` is commented out — 50 red. Before this release, Step 8 printed nothing at all.
...
- **(b)**: 2 `off_shipped_path`, 2 `reachability_unknown`. The QR-sheet finding names exactly the defect: the evidence (`infra/make-qr-sheet.ts`, `tests/unit/qrSheet.test.ts`) is not reachable from the entry point users go through (`src/app/admin/page.tsx`).
