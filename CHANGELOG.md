...
Tests: `tests/test_claude_cli_permissions.py`, `tests/test_operational_e2e_audit.py`, `tests/test_brownfield_js_dogfood.py` follow the new default; the "respects an explicitly-passed `--effort`" case deliberately still asserts `max`. `codd init` was run for real into three fixture projects (fresh TS, fresh Python with `--requirements`, and a project with a pre-existing `.gitignore` and `--config-dir .codd`) to confirm template substitution, the canon notice, and the never-clobber rule — a check pytest alone cannot make. Suite: 7470 passed, 0 skipped, 6 pre-existing failures unchanged from v3.37.0 (verified identical on a clean tree).
