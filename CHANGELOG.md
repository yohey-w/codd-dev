...
- **`codd/runtime_record.py` (NEW) — the runtime execution ledger.** The runtime smoke runner writes `<codd-dir>/runtime_ledger.json` when the stage is enabled **and at least one check actually ran**; an all-skipped run and a disabled stage write nothing, because a record of nothing is not a record. It holds `recorded_at` (UTC), `passed`, each check's `name` / `category` / `passed` / `skipped`, and `config_digest` — a hash of the project's own `runtime_smoke` + `runtime` configuration as it stood at run time. Missing or malformed reads as **no evidence**, never as evidence, the same rule `acceptance_ledger.json` already follows. Like that ledger it is a project artifact meant to be committed and read in diffs; unlike it, it is written automatically, and that difference is the point — a manual record is a human verdict that only a human may refresh, this one records that a machine ran, which only the machine that ran can honestly write.
- **`runtime_evidence_not_executed` (NEW finding).** The stage is on but nothing recorded a run of it: no ledger (`no_record`), a ledger written against a runtime configuration the project has since changed (`config_changed`), a ledger past the project's freshness window (`stale`), or — for an explicitly declared `verified_by: runtime:<case>` — a ledger in which no check answers to that name (`target_not_executed`). `runtime_evidence_not_executable` is unchanged and still means the other thing: the stage is off. Two holes, two remedies, two findings.
- **The criterion goes back to `unbound_acceptance` as well.** An enabled-but-never-run obligation now binds exactly as much as a switched-off one: nothing. That is not double-reporting — "run the stage" and "write a test" are different remedies for the same criterion, which is the rule this module already applied on the switched-off side.
- **Explicit and inferred targets are treated differently, as they already were.** An explicit `runtime:<case>` names a case, so a run in which no non-skipped check answers to that name has not discharged it (`print_sheet` / `Print Sheet` / `print-sheet` are one name, not three). An **inferred** obligation — an `operation_flow.<id>` reference in a project that never adopted the `verified_by` column — declares no such correspondence, so any recorded run satisfies it. CoDD does not invent a mapping the project never wrote down. A run that FAILED still counts as executed: Step 8 already reported that in red.
...
Four holes, one shape: **the acceptance criterion was never connected to anything a machine runs.** CoDD reconciled design against implementation and test documents against test markers — but never the criterion itself, and `test_coverage` treated "this project declares no verifiable behaviors" as a pass, which made the empty registry the safest registry.

**The invariant** (`docs/design/acceptance-evidence-invariant.md`): for every acceptance criterion AC there is evidence E such that **(a)** E is machine-executed (or an owner-attributed `manual` record), **(b)** E runs through the **shipped path** — the code reachable from the entry point a user touches, **(c)** E references AC's **named parameters** instead of a retyped literal, and **(d)** E is bound to the **content** of the implementation it verified. Anything missing is a **finding** — the "print a notice and pass" branch is gone.

**This release adds a NEW CHECK. It does not change the verdict of any existing project.** `acceptance_evidence` is **advisory (amber) by default**: it reports exactly the findings strict mode reports, and fails nothing. `acceptance_evidence.mode: strict` makes them red. That split is deliberate — the two ways a new check can be wrong are opposite (fail a project on an upgrade nobody asked for, or stay so quiet the hole is invisible again), and advisory refuses both. `codd init` writes `mode: strict` for new projects, which have nothing to retrofit; CoDD's own repository runs strict.

- **`codd/acceptance_evidence.py` — the parser/resolver.** Acceptance criteria are read from the project's EXISTING requirement tables, using the same documents and the same in-scope rule `requirement_reconciliation` already uses. A table contributes criteria only when one of its COLUMNS is an acceptance column (built-in English + Japanese header vocabulary, extended — never replaced — by `acceptance_evidence.acceptance_columns`). A table that lists requirements without ever stating acceptance declares nothing and stays silent; inventing an obligation per row would flood a brownfield project and train its owners to ignore the check. The machine-readable declarations `verified_by` / `params` / `critical` are OPTIONAL EXTRA COLUMNS of that same table, adopted incrementally; a project that has adopted none is still audited through the anchors it already has (`operation_flow.<id>` references, requirement-id tokens in source, VB coverage markers).
- **`codd/dag/checks/acceptance_evidence.py` — the gate**, registered and added to `verify_runner.DEFAULT_CHECKS`, so it runs inside plain `codd verify` as well as `codd dag verify`. Findings: `runtime_evidence_not_executable` (a runtime obligation in a project whose `runtime_smoke` is off — the silent skip), `unbound_acceptance`, `vacuous_evidence`, `unresolved_evidence`, `manual_evidence_missing`, `vb_registry_missing`, `stale_manual_evidence`, `param_not_referenced`, `undeclared_numeric`, `off_shipped_path`, `multiple_implementers`, `reachability_unknown`, `critical_manual_only`. Amber by default; red under `mode: strict`; every class individually overridable. A project that declares no criteria reports `skip` with `checked_count == 0`, which the materiality overlay keeps distinct from a verified clean run.
- **A marker is a claim; an assertion is evidence.** A `codd: covers vb=` line over an empty test, a test that is skipped (`test.skip`, `it.skip`, `xit`, `@pytest.mark.skip`, `#[ignore]`, `@Disabled`), or a file that merely mentions the requirement id in a comment — including one that mentions it to say the opposite, `// R-1 NOT implemented` — does NOT bind the criterion. It is reported as `vacuous_evidence` and the criterion stays unbound. Comments are stripped before any of this is measured, so an assertion written inside a comment never counts.
- **`test_coverage.require_vb_table` (NEW).** Decides whether a missing VB registry also FAILS the implement-time coverage gate. Unset ⇒ follows `acceptance_evidence.mode` (strict fails, advisory reports only), so an upgrade breaks nothing. The missing registry is reported either way; only an explicit `require_vb_table: false` — the project declaring it verifies its criteria some other way — silences the finding.
- **`codd acceptance list | sync | record`.** `sync` derives one VB row per acceptance criterion into the canonical registry, deterministically, no AI: append-only, id-stable (`VB-<req-id>`), idempotent. `record` writes a MANUAL verdict into `<codd-dir>/acceptance_ledger.json` bound to the content hashes of the implementation it accepted — **the anchored files plus their import closure**, so the accepted behaviour cannot move one file sideways and keep the verdict looking current. The ledger is never refreshed automatically; a self-updating ledger detects nothing.
- **(c) is a NAME check, not a number grep.** The defect is a value copied three times, and a copy cannot notice that the original moved. Grepping the criterion's digits would catch it only by luck and would fire on every date, version and paper size on the way. Declare `params: per_sheet=15` beside the criterion and read `per_sheet` in the test. A bare literal in a criterion that declares no parameters is a nudge to declare, not an accusation — the tokenizer rejects every number that is part of a larger token (`F-E2`, `A4`, `v1.7`, `2026-08-22`, paths), which is where the false positives live.
- **(b) asks its question about EVIDENCE.** Not about every file that mentions a requirement id: requirements legitimately span layers, and a migration being unreachable from a page is architecture, not a defect. The evidence for a criterion is the tests bound to it plus the tests of any file claiming its id; if none of them, nor anything they exercise, appears in the import closure of the requirement's entry point, that is `off_shipped_path`. Entry points come from the criterion's own `operation_flow.<id>` — the operation's `route:` resolved through the project's declared `filesystem_routes`, or an explicit `entry_file:` that always wins over inference. An entry that cannot be resolved is reported as `reachability_unknown` rather than guessed at in either direction.
- **`.codd/dag.json` no longer records an absolute `project_root`** (now `"."`). An absolute path baked the builder's machine into a committed artifact, so the same tree serialized on two machines produced two different files.

Known limits, stated in the README rather than left to be discovered: the check asks whether the test attached to a marker runs and asserts, never whether the assertion is meaningful (`expect(true).toBe(true)` binds); and both the shipped-path closure and the freshness hash follow static imports only, so a computed specifier is invisible to both. CoDD holds itself to the invariant — `docs/requirements/system-requirements.md` §6 states one acceptance criterion about this check's own advisory behaviour, `docs/test/test_strategy.md` declares its VB, and the test that proves it carries the marker.

Tests: 71 (`tests/dag/test_acceptance_evidence.py`), all `tmp_path` fixtures — synthetic miniatures of the failing shape, no file copied from any real project, no framework or project literal in the check. They include the adversarial inputs an independent review used to break the first draft: a marker over an empty test, a marker over a skipped test, a requirement id in a comment that denies the implementation, and a value edited one import away from the file the verdict was recorded against.

Dogfood — the real project, at the commit BEFORE the fix, with the requirement documents untouched (69 acceptance criteria parsed from its existing tables), run with `mode: strict`:

- **(a) demotion**: 50 criteria declare runtime obligations and `runtime_smoke` is commented out. Before this release, Step 8 printed nothing at all.
- **(a) wiring**: 57 criteria reach no machine-checked evidence, plus `vb_registry_missing` (the project owns no VB registry, so not one criterion was reconciled against a test marker). The QR-sheet requirement is in both lists.
- **(c)**: 41 criteria state bare literals with no `params:` — including the requirement whose "15" disagreed with the code's 20.
- **(b)**: 2 `off_shipped_path`, 2 `reachability_unknown`. The QR-sheet finding names exactly the defect: the evidence (`infra/make-qr-sheet.ts`, `tests/unit/qrSheet.test.ts`) is not reachable from the entry point users go through (`src/app/admin/page.tsx`). At the commit AFTER the fix, that finding and the criterion's `unbound_acceptance` both clear.
- **Measured false positive**: 1 of the 2 `off_shipped_path` findings is anchored on a comment that names a requirement id in order to say the file does NOT implement it. A negative mention reads as a claim. Recorded rather than papered over; it is the argument for (b) staying amber.
- A second brownfield project (prose requirements, no acceptance columns in any table) produced **0 findings**: the check went dormant, as designed. That measures the scoping rule, not the finding logic.

## [3.40.0] - 2026-08-11 — Feature: a verification budget you can raise for ONE run, and a skip that can no longer masquerade as a pass

A skipped verification node reports neither pass nor fail. That is the whole hole. A project with a deliberately tight `verify.verification_timeout` — the fast-fail budget a slow dev machine needs so a hung browser test does not eat twenty minutes — runs a handful of nodes, exhausts the budget, skips the rest, finds zero failures, and exits 0. **Green by absence.** And the only way to ask "what would a complete run say?" was to edit the committed budget, which mutates the project's contract to answer a one-off question.

Both halves of that are fixed, and they are two halves of one thing: the run-scoped intent had no entry, so it had to be smuggled in through the permanent policy.

- **`codd/repair/verify_runner.py` — run-scoped entry for both timeout bounds.** `CODD_VERIFICATION_TIMEOUT_TOTAL_SECONDS` and `CODD_VERIFICATION_TIMEOUT_PER_NODE_SECONDS` override `verify.verification_timeout.{total_seconds,per_node_seconds}` for one invocation. Unset or blank ⇒ the committed value, byte-identical to before. **Present but not a positive number ⇒ an honest error naming the variable** — a typo'd budget must never degrade quietly into "no override", which is the silent-fallback pattern this codebase rejects everywhere else.
  - **Why an environment variable and not a CLI flag.** The embedded verifications have no command line of their own. `codd stack update-lock` runs an ordinary verify (`cli.py`), and so do the repair loop and the greenfield autopilot; a flag on `codd verify` reaches none of them. Resolution therefore happens at `_verification_timeout_seconds`, the single choke point they all share. It is also the same shape as the existing `CODD_CDP_BASE_URL`, and it leaves `codd.yaml` untouched — which matters when the config feeds a hash (measured below).
- **`verify.require_complete_verification` (default `false`) — the verdict rule.** Turned on, a run that skipped verification nodes **nobody asked it to skip** is a `verification_coverage` FAILURE. "Passed" then means "everything this project declared verifiable was verified", not "nothing that happened to run said no". Run-scoped entry: `CODD_VERIFY_REQUIRE_COMPLETE_VERIFICATION`.
  - **The discriminating distinction is the skip's origin, not its count.** `_skipped_result` already recorded a `skip_reason`, so a nodes-excluded-on-purpose skip (`--runtime-skip verification-test`) is separable from an involuntary one (`total_timeout_exceeded`) with no new data. A declared exclusion is a **contract** and never trips the rule; only the involuntary gap does. That is why the setting is a boolean and not a threshold: "N skips are acceptable" is a policy number no project asked for, and inventing one would add config surface to express something nobody declared.
  - **Off by default, deliberately.** Clipping a long tail is a legitimate policy for some projects, and a new setting must never silently change an existing one. The failure is marked `code_addressable: false` / `environment_build_error`: an unexecuted node is not a code defect and must never be routed to the repair engine as one.
- **Visibility is NOT opt-in.** Independently of the rule and of the verdict, a run that skipped nodes involuntarily now says so: *"53 of 65 verification node(s) were skipped without being asked to (total_timeout_exceeded=53); only 12 actually executed"*, plus how to raise the budget and how to make the state fail. Reporting what happened is not a behavior change; only the verdict was gated.
- **Deliberate boundary.** `STRUCTURAL_ONLY_WARNING` warns on plain `verify` and escalates to a stage failure in the greenfield autopilot. Mass-skip does **not** escalate there. That asymmetry is a scope decision, not an oversight: the coverage rule is new, and arming it silently for autopilot builds would change verdicts nobody opted into. It can be demanded per run today via the environment entry.

Tests: 14 new (`tests/test_verify_runner_timeout.py`, T07-T19), all `tmp_path` + `monkeypatch`, no framework or project literal. They pin the control (committed budget still clips when the env is unset), the override in both directions (env wins; env `off` beats a committed `true`), both malformed-value errors — including a malformed boolean on a run with **nothing** to skip, so an operator who believes the rule is armed cannot get a green that never evaluated it — blank-is-inert, the always-on warning on an otherwise-**passing** run, the coverage failure and its details, the declared-`--runtime-skip` negative control, and the CLI surfacing the warning. Suite: **7588 passed**, 0 failed, **0 skipped** (7574 before + 14).

Dogfood — a real Next.js/Playwright project, 65 verification nodes, **red-before-green on the same project, config untouched in both runs**:

- **Run A** (committed 120s budget, rule on): 7 PASS / 5 FAIL / **53 SKIP** `(total_timeout_exceeded=53)`. The warning printed; `verification_coverage` entered the failure set. On the old build the same 53-node hole was invisible to the verdict.
- **Run B** (`CODD_VERIFICATION_TIMEOUT_TOTAL_SECONDS=10800`, `..._PER_NODE_SECONDS=300`, rule on): **44 PASS / 21 FAIL / 0 SKIP** over 21m34s. Every node the budget had been hiding ran, and the verdict became a statement about all 65. Per-node mattered as much as total here: this project's Playwright nodes declare no template-level timeout, so the committed 30s per-node cap was cutting whole spec files short.
- **Run C**, same budget after provisioning the fixtures the project seeds only under a CI-only flag: **47 PASS / 18 FAIL / 0 SKIP** over 27m36s. The delta is the point — three nodes went from red to green on *provisioning*, which is a distinction the old 53-node skip hole could not express at all.
- **The candidate stack-lock fingerprint was `sha256:ec91be25…` before and after the upgrade** — identical to the value recorded when this project's lock was last reviewed. The environment entry changes no committed byte, so nothing hashed off the project's configuration moves.

The dogfood project did **not** reach a green full verification, and that is reported rather than papered over. What the newly-visible red turned out to be is itself the argument for the feature: **9 of the 18 fail only on a viewport profile the project's own CI never runs** (its scripts pin one Playwright project; CoDD's template runs every configured one), several more want credentials its CI supplies through a separate test slot, and one is a genuine conflict between the app's login rate limit and the login rate of its own suite from a single address. None of that is CoDD's to decide, and every candidate remedy narrows what gets verified — which is exactly the move a run reporting `0 SKIP / 65 total` makes impossible to do quietly.

No framework, language or project literal enters the new paths: the mechanism keys on a skip reason the runner already recorded.

## [3.39.0] - 2026-08-03 — Feature: canon integrity tripwire — a formatter can no longer rewrite an approved requirements document and keep every check green

v3.38.0 shipped a `.prettierignore` from `codd init`. That is a **first layer, and it is narrow in two ways**: it names one tool, and it only ever reaches a NEW project. It does nothing about dprint, `markdownlint --fix`, an IDE's format-on-save, a stray bulk `sed`, or an agent editing the wrong file — and nothing at all for the repositories that already exist. This release adds the tool-independent layer.

- **`codd/canon.py` (new) — the ledger.** A SHA-256 of every canon document, recorded into `<codd-dir>/canon.lock` at the moment a human accepted it. Raw bytes, never decoded text: the claim being protected is byte-identity with the approved original, so an encoding or line-ending change must register as a change. The payload carries no timestamp and no tool version, so an `accept` that changes nothing produces no diff.
- **`codd/dag/checks/canon_integrity.py` (new) — the check.** Registered as `canon_integrity` and run by `codd dag verify` and by the `codd check` aggregation (both verified by running them). It is **not** in `deployer._collect_dag_completeness_gate`'s hardcoded six-check list, so `codd deploy` is unaffected — that list is a structural-DAG subset, and widening it is a separate decision. An **altered or missing** canon document fails at `canon.severity`, default **red**. Severity is deliberately split: an **untracked** in-scope document is **amber**, because adding a file is already visible in `git status` and hard-failing it would turn every greenfield run red the moment the first requirements file is written — a gate that fires on normal work is a gate people learn to bypass. The untracked signal is also how a reference copy dropped into `docs/requirements/` (the v3.38.0 double-counting trap) surfaces.
- **`codd/cli.py` — `codd canon status` / `codd canon accept --for <work reference>`.** `accept` is the ONLY writer of the ledger. Nothing refreshes it automatically: **a self-updating ledger detects nothing**. It is built so it cannot be a reflex: the work reference is **required** (an argument-less invocation exits 2 and the ledger does not move), the actual **content diff** of what is about to be blessed is printed (`git diff --stat` plus the patch, not merely a list of filenames), and that reference is written into the committed ledger next to each digest — so an acceptance with nothing behind it stays visible in `git log` and `git blame`. An entry whose digest did not change keeps the reference of the accept that originally recorded it, so an accept touches only the lines it actually changes.
- **Honest scope, stated in three places.** This is **not an authorization gate**. It detects *unaware* change — a formatter, a format-on-save, a bulk `sed`, an agent writing to the wrong path, a bad merge — precisely because nobody runs `accept` for a change they do not know happened. A **deliberate edit followed by `accept` is green, by design**, and no setting alters that; against a deliberate bad edit the defences are the recorded work reference and code review, not this check. The limit is written at the top of `codd/canon.py`, at the top of the check module, and in the header of every `canon.lock` file, and a test asserts all three. The agent-facing rule — *accept only when changing the document WAS the work you were asked to do; accepting to turn a red check green is the one use that defeats the mechanism* — is printed by `canon accept` (including when it refuses), by the failing check, and by the hook rejection.
- **Merge-friendly ledger format (v2).** One document per line, sorted by path, blank-line separated, with no enclosing JSON structure whose last element churns on every append. Two branches accepting **different** documents merge with no conflict; two branches accepting the **same** document still conflict, which is correct. The blank separator is measured rather than cosmetic: Git conflicts when the two sides' changed regions touch, and without it two documents whose ledger lines happened to be neighbours (`alpha.md` / `beta.md`) still collided — both the far-apart and the adjacent case are pinned by tests, along with the same-document negative control. A v1 JSON ledger written by an earlier build is still read, so upgrading never resurrects the "no ledger" advisory for a project that had already adopted the mechanism.
- **CoDD's own generated output is never ledgered by discovery.** `codd require --audit` writes `coverage_audit_report.md` and its `--output` **defaults to `docs/requirements/`** — squarely inside the default canon scope. Ledgering it would have turned the tripwire red after an ordinary audit run, which is exactly the alarm-fatigue failure the narrow scope exists to avoid, landing on CoDD's own dogfood repository. It is skipped during discovery only; an explicit `canon.docs` entry still wins, because that is a deliberate declaration.
- **`codd/hooks/__init__.py` — the commit-time half.** `run_pre_commit` now rejects a commit whose canon documents no longer match the ledger, naming both ways out (`codd canon accept` for an intended change, `git checkout --` for an unintended one). It evaluates the **working tree**, not the index: `prettier --write .` damages the tree wholesale while the user commits something unrelated, and a gate that only looked at staged paths would wave that through and let the corruption ride in on a later commit. The tripwire degrades to non-blocking on any internal failure — a broken guard must never make a repository uncommittable.
- **Backward compatible by construction.** A project with no ledger is an **amber advisory with adoption guidance**, never red, and the hook does not block it: upgrading CoDD must not break an existing repository. `SKIP` is reserved for its documented meaning ("dormant / unconfigured") and is used only when there is genuinely nothing to protect. `codd init` now seeds the ledger so a new project is armed from its first commit; `codd init --requirements` into an **existing** project creates one only when absent — refreshing it there would silently absorb drift that is already present.
- **`codd/defaults.yaml` + `codd/dag/defaults/{web,cli,mobile,iot}.yaml` — the check is actually selected.** `dag.enabled_checks` is an allowlist, so a newly shipped check is a silent no-op for every project on a pinned project-type profile. All four pinned profiles now select `canon_integrity` (Markdown under `docs/` is language- and project-type-independent), and a test asserts it for each — a profile that quietly ran without the tripwire would reproduce the exact invisible-damage signature the feature exists to remove.
- **Scope is deliberately narrow, and that is the load-bearing decision.** The ledger covers *requirement* documents (the discovery `requirement_reconciliation` already uses, honouring its pinned `docs:` list), not every `*.md` under `scan.doc_dirs`. CoDD's own AI stages — `generate`, `propagate`, `fixup-drift`, `extract` — legitimately rewrite design and test documents; ledgering those would leave the check red after every normal run and train users to hit `accept` reflexively. **A detector dead by alarm fatigue is worse than no detector.** `canon.docs` widens the scope for a project whose canon is broader. `scan.exclude` applies in both cases, so reference material under `docs/source/` is not canon — symmetric with the v3.38.0 F-8 fix, and reusing the same `discovery` helpers rather than reimplementing glob semantics (reimplementing them *was* F-8).

Tests: 46 new (`tests/test_canon_integrity.py`). **Red-before-green on the existing surfaces, not merely on a missing import.** On v3.38.0, in a fixture project on both a pinned (`web`) and an unpinned (`generic`) profile, reformatting a requirements table exactly the way a Markdown formatter reformats it changed **no check's verdict at all** in `codd dag verify --format json` and the pre-commit hook exited 0 — the corruption left no trigger. On the new source the same edit flips `canon_integrity` from `(pass, True, red)` to `(fail, False, red)` and the hook exits 1, on both profiles. Suite: **7521 passed**, 0 skipped (7475 at v3.38.0 + 46 new); the 6 failures are pre-existing and were verified identical on a clean HEAD worktree rather than assumed. The suite also holds the negative controls: an untracked document is amber and never downgrades a simultaneous alteration; an excluded document is not canon while the same document with an empty `exclude` IS; running the check twice never rewrites the ledger; a corrupt or future-versioned ledger is red rather than a silent pass; a symlink escaping the root is never ledgered; and the hook's in-code defaults are asserted equal to `defaults.yaml` (the hook reads a RAW `codd.yaml`, unmerged, so a default living only in `defaults.yaml` would make it silently see `canon` as absent).

No language, framework or domain literal enters the new paths: scope comes from the project's own requirement-document declaration plus `scan.exclude`, and the comparison is a content digest.

## [3.38.0] - 2026-08-03 — Fix: `codd init` no longer scaffolds a project that silently corrupts its own canon (from a greenfield dogfood run)

A real greenfield project built end-to-end on `codd init` hit five defects, all of them in the scaffold rather than the engine. Every one shares a failure signature CoDD exists to eliminate: **the damage leaves no trigger to notice it.**

- **`codd/templates/prettierignore.tmpl` (new) + `codd/cli.py` — formatters no longer silently break CoDD's canon.** A requirement unit is a Markdown table ROW (`parse_requirement_units`), so a formatter that only re-aligns table pipes changes no meaning, passes review, and keeps every check green — while destroying byte-identity between the human-approved original and the document CoDD reconciles against. Observed in the field: `npx prettier --write .` rewrote **440 lines** of a requirements table; it was caught only because an unrelated diff check happened to run. `codd init` now writes a `.prettierignore` excluding `docs/requirements/`, `docs/source/`, and the config dir. Written unconditionally, not gated on language: the artifact at risk (Markdown under `docs/`) is language-independent, so no language literal enters this path, and the file is inert where Prettier is unused.
- **`codd/templates/project_gitignore.tmpl` (new) + `codd/cli.py` — a scaffolded project no longer starts with zero secret hygiene.** `codd init` created `<config_dir>/.gitignore` (CoDD's own scan/report output) but never a project-root one, so the first commit of a fresh project could carry `.env`. A conservative root `.gitignore` is now written. Deliberately narrow patterns (`.env`, `*.pem`, `secrets.json`, ...) rather than a broad `*secret*` glob — an over-wide ignore rule silently untracks legitimate source, which is the same class of invisible damage being fixed here.
- **Both root files are written ONLY when absent**, with an echoed `already exists — left untouched` line. `codd init` commonly runs after a scaffolder (`create-next-app`, `poetry new`) that wrote its own; clobbering a user's `.gitignore` would be a worse regression than the gap being closed.
- **`codd/cli.py` + `codd/templates/codd.yaml.tmpl` + `RUNBOOK.md` — the double-counting trap in `docs/requirements/` is now stated where the user is looking.** Discovery rglobs `docs/requirements/**/*.md`, so parking a reference copy of the same document beneath it (an `original/` or `reference/` folder) doubles every requirement unit **with no warning** — and the reader has nothing to judge whether the larger count was right. `codd init --requirements` now prints the canon rule, the `codd.yaml` template carries it above the reconciliation block, and the RUNBOOK documents the escape hatch (`requirement_reconciliation.docs` to pin the canonical files explicitly). No new check: discovery happens down three independent paths (`requirement_reconciliation`, `required_artifacts_deriver`, `requirement_completeness_auditor`), and a warning wired into one of them would read as complete coverage while being partial.
- **`codd/templates/codd.yaml.tmpl` — `requirement_reconciliation` now ships as a commented block.** Its default scan target is `docs/requirements/**`, which is exactly where `codd init --requirements` puts files, yet the generated config never mentioned it — so a user had no way to discover the requirement↔operation reconciliation mechanism short of reading `codd/requirement_reconciliation.py`. Mirrors how `operation_flow` and `runtime` are already surfaced.
- **`--effort max` is no longer the baked-in default** (`codd/claude_cli.py` `DEFAULT_CLAUDE_EFFORT`, `codd/defaults.yaml`, `codd/templates/codd.yaml.tmpl`, `codd/ai_invoke.py`, `codd/cli.py`, `codd/extract_ai.py`) — now `xhigh`. CoDD's AI stages run long, distractor-heavy, tool-driven contexts, which is precisely the regime where a larger reasoning budget is not a strict upgrade: accuracy can DECREASE with more test-time compute on distractor-rich tasks, and lower "overthinking" correlates with better agentic outcomes. `max` remains correct for a narrow, closed problem and is still honoured whenever a caller passes `--effort` explicitly (an existing option always wins in `_ensure_option`). `docs/requirements/system-requirements.md` FR-AI-1, which tracks this default as `[observed]`, moves with the code.
- **`RUNBOOK.md` — "local green does not prove deployed green" is now a Phase 5 release condition.** A disposable local PostgreSQL verified RLS and public functions end-to-end and went fully green; production still returned zero rows for every user, because the managed platform installs extensions into a different schema than a stock engine — so pinning a `security definer` function's `search_path` (correct in itself) hid the function in production only. The failure surfaced as *an empty result*, indistinguishable from success. "Verified locally" and "verified on the deployment target" are different claims and only the second grounds delivery; the runbook now requires one real verification on the deployment target, kept as a re-runnable script in the repo.

- **`codd/hooks/__init__.py` + `codd/validator.py` + `codd/templates/codd.yaml.tmpl` — `scan.exclude` now applies to document discovery, so the `docs/source/` recommendation above is actually usable.** Both gates ignored `scan.exclude` and walked `scan.doc_dirs` alone, so a Markdown file the project had explicitly excluded from scanning still had to carry CoDD frontmatter — the commit gate contradicted the config. That lands squarely on reference material parked under a doc dir (a received customer spec, a vendored upstream document, a past version): foreign text that will never have frontmatter, where excluding it was the documented way to say so. Reproduced end-to-end before the fix — a fresh `codd init` project with `docs/source/original-spec.md` could not be committed once the pre-commit hook was installed, meaning CoDD's own remedy for the double-counting trap was blocked by CoDD's own gate. The template now ships `docs/source/**` in `scan.exclude`. Both call sites reuse `discovery.scan_exclude_patterns` / `matches_exclude_pattern` rather than reimplementing glob semantics; behaviour is unchanged when no `exclude` is configured.

Tests: 5 new (`tests/test_validate.py`, `tests/test_hooks.py`) covering the excluded path, the **negative control** that a non-excluded frontmatter-less doc is still rejected by both gates, and the opt-in property that an empty `exclude` checks everything — 3 of them proven RED on the prior source before the fix. `tests/test_claude_cli_permissions.py`, `tests/test_operational_e2e_audit.py`, `tests/test_brownfield_js_dogfood.py` follow the new default; the "respects an explicitly-passed `--effort`" case deliberately still asserts `max`. `codd init` was run for real into three fixture projects (fresh TS, fresh Python with `--requirements`, and a project with a pre-existing `.gitignore` and `--config-dir .codd`) to confirm template substitution, the canon notice, and the never-clobber rule — a check pytest alone cannot make. Suite: 7475 passed, 0 skipped, 6 pre-existing failures unchanged from v3.37.0 (verified identical on a clean tree). `codd dag verify` on codd-dev itself: exit 0, red=0.

Scaffold, config-template, docs and one default value — no language, framework, or domain literal introduced.
