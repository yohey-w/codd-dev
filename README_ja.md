...
- **anti-false-green はコアが所有する** — 「偽の成功を許さない」という不変条件はコアに置かれる。プロファイルはパラメータを設定できるが、それを**弱めることは決してできない**。（実際の Next.js アプリでエンドツーエンド検証済み — [`dogfood/v3_nextjs_live_e2e.md`](dogfood/v3_nextjs_live_e2e.md) を参照。）
