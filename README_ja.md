...
artifact contract を有効にすると、`codd plan/generate/implement/verify` はコマンドが単に返ったことではなく、実際に産出された artifact によって完了を判定する — 非力なモデルでも北極星に留まらせる、ハーネスレベルの仕組みである。
...
# Greenfield: 要件を書いたら、あとは放置
codd greenfield --requirements docs/requirements/requirements.md
```

この 1 コマンドが無人パイプライン全体 — init → elicit → plan → generate → implement → verify (auto-repair) → propagate → check — を実行する。ユニットごとにチェックポイントを記録し (`codd greenfield --resume` で中断地点から再開)、`--ntfy-topic` で進捗通知、`--dry-run` でプレビュー。同じパイプラインはどの環境でも使えるよう 3 つの形態でも提供する: 透明なシェル合成 ([examples/greenfield_autopilot.sh](examples/greenfield_autopilot.sh))、Claude Code Agent Workflow テンプレート ([examples/claude_workflows/codd-greenfield.js](examples/claude_workflows/codd-greenfield.js))、そして `codd-greenfield` スキル (`codd skills install codd-greenfield --target both` — Claude Code と Codex CLI の両方で動作)。
...
`codd fix [PHENOMENON]` は CoDD の2つ目の入口。自然言語で変更要望を伝えると、CoDD が関連設計書を lexicon + 意味類似スコアで特定し、LLM が更新する。そこから変更は **設計 → 実装 → テスト → 検証** へと波及する。Stage 4 が DAG から影響を受ける実装ファイル・テストファイルを決定論的に解決し、allowlist に限定した LLM patch を当て、verify ゲート (新規 red の DAG チェックなし + ローカルテスト合格) に通す。失敗時は targeted rollback (この実行で書いたファイルのみ revert)。`--no-propagate-impl` で opt-out、`--propagate` を付ければ依存する設計書も整合させる。`--dry-run` でプレビュー、`--non-interactive` で CI 実行可。
...
| コマンド | 一言で |
| --- | --- |
| 🚀 **`codd greenfield --requirements FILE`** | 無人オートパイロット: 要件 in、システム out。全ゲート自動承認、`--resume` でチェックポイント再開、ntfy は通知のみ。構成部品 (`generate --all-waves` / `implement list-tasks` / `verify --repair-mode`) は単体でも使える。 |
...
| 🎯 **`codd fix`** / **`codd fix [PHENOMENON]`** | 2 モード — CI 失敗の自動検出、または自然言語での変更要望。PHENOMENON モードは 設計 → 実装 → テスト → 検証 まで波及 (`--no-propagate-impl` で opt-out)。 |
| 🧭 **`codd operations {derive,show,approve,apply}`** | どの宣言済み operation にも紐づかない要件ユニットを検出し、AI が `operation_flow` エントリを提案、人間レビュー用に proposal を書き出す — `approve` + `apply` までは `codd.yaml` に何も書かない。opt-in・非破壊。（旧 `merge` は `apply` の非推奨エイリアスとして引き続き利用可。） |
| 📜 **`codd contract {derive,show,apply,verify}`** | V 字モデルの artifact contract。`derive`/`apply` は「プロジェクトが stage ごとに *どの* カタログ artifact を宣言するか」を要件駆動で選定する（`derive` はプロジェクトのシグナル — 要件ドキュメント・設計ドキュメント・lexicon・宣言済み `operation_flow`・ソースディレクトリ・test/e2e スイート — を検出し、レビュー可能な提案を書き出す。`codd.yaml` には一切触れない。`apply` がそれを非破壊・冪等にマージし、`--enable` でゲートを有効化）。`verify`/gate は完了側: contract を有効にすると、各 stage は宣言した artifact が産出/妥当化されるまで完了を主張できない。`show` はカタログ + プロジェクトの contract を描画。opt-in、contract 不在/無効なら no-op。（旧 `suggest`/`adopt` は非推奨エイリアスとして引き続き利用可。） |
...
| ゲート | 役割 |
| --- | --- |
| 🧪 **`codd verify --runtime`** | Step 8 ランタイム smoke (DB 起動 + dev server 疎通 + smoke HTTP + 実ブラウザ E2E + `runtime.crud_flow_targets` による opt-in CRUD 反映検証 + `runtime.action_outcome_targets` による action outcome coverage)。`--runtime-skip` でカテゴリ別 skip + 理由を report に記録。 |
...
- Claude Code 要件変更ナッジレシピ (`claude_requirements_nudge.json`) — `docs/requirements/` 配下のファイルが編集されたとき「要件が変わった → `codd greenfield --resume` か `codd check` を実行」と助言を表示するだけ (print のみ。高コストなパイプラインを勝手に実行することはない)
