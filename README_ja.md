...
| 🧭 **`codd operations {derive,show,approve,merge}`** | どの宣言済み operation にも紐づかない要件ユニットを検出し、AI が `operation_flow` エントリを提案、人間レビュー用に proposal を書き出す — `approve` + `merge` までは `codd.yaml` に何も書かない。opt-in・非破壊。 |
| 📜 **`codd contract {show,suggest,adopt,verify}`** | V 字モデルの artifact contract。`suggest`/`adopt` は「プロジェクトが stage ごとに *どの* カタログ artifact を宣言するか」を要件駆動で選定する（`suggest` はプロジェクトのシグナル — 要件ドキュメント・設計ドキュメント・lexicon・宣言済み `operation_flow`・ソースディレクトリ・test/e2e スイート — を検出し、レビュー可能な提案を書き出す。`codd.yaml` には一切触れない。`adopt` がそれを非破壊・冪等にマージし、`--enable` でゲートを有効化）。`verify`/gate は完了側: contract を有効にすると、各 stage は宣言した artifact が産出/妥当化されるまで完了を主張できない。`show` はカタログ + プロジェクトの contract を描画。opt-in、contract 不在/無効なら no-op。 |
