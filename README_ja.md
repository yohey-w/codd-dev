...
- **v2.15.0 (現在)** — `kind: common` 導入 (cmd_467)。共有インフラ (DB クライアント・middleware・設定ファイル等) を `common_node_patterns:` glob または frontmatter で分類。C5 amber −79.2% (125 → 26)。`**` glob translator 修正。詳細は [CHANGELOG](CHANGELOG.md)。
...
- **v2.16.0 (予定)** — `codd fix [PHENOMENON]` — 直したい事象を自然言語で渡すと、関連設計書を特定・更新し、実装・テストを更新して DAG verify まで全自動。インタラクティブ応答付き。
