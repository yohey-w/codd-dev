...
- **v2.16.0 (現在)** — `codd fix [PHENOMENON]` 北極星 入口2 の実装 (cmd_468)。直したい事象を自然言語で渡すと、Tier-1 lexicon + Tier-2 セマンティックスコアで関連設計書を特定・LLM で更新・DAG verify ゲート通過まで全自動。HITL インタラクティブ (候補選択/曖昧明確化/リスク確認) + `--non-interactive` (CI 用)。新規 66 tests、合計 2908 PASS、SKIP=0。詳細は [CHANGELOG](CHANGELOG.md)。
...
- **v2.17.0 (次期)** — PHENOMENON の impl/test 自動波及完成 (AC #8)、Codex wrapper。
