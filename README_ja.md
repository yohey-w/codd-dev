...
- **v2.13.0 (現在)** — opt-out 保護: `OptOutPolicy` により、すべての設定レベル opt-out (`ci.provider=none` 等) に `justification` + `expires_at` が必須化。サイレント SKIP 廃止、severity 維持。`codd validate` がポリシー違反を報告。
- **v2.12.0** — テスト完全性ゲート: C7 `actors_without_journeys` amber promotion + C8 `ci_health` 静的チェック新設。
- **v2.11.0** — Sprint 廃止 implement (`--design <path> --output <dir>` 直接指定)。
- **v2.14.0 (予定)** — C8 `ci_health` ランタイムモード (opt-in `ci.runtime_check: true`)。
