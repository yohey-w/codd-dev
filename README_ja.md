...
| ゲート | 役割 |
| --- | --- |
| 🧪 **`codd verify --runtime`** | Step 8 ランタイム smoke (DB 起動 + dev server 疎通 + smoke HTTP + 実ブラウザ E2E + `runtime.crud_flow_targets` による opt-in CRUD 反映検証)。`--runtime-skip` でカテゴリ別 skip + 理由を report に記録。 |
