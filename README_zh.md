...
- **v2.16.0 (当前)** — `codd fix [PHENOMENON]` 北极星入口2 实现 (cmd_468)。用自然语言描述想修复的现象，CoDD 通过 Tier-1 lexicon + Tier-2 语义评分定位相关设计文档、LLM 更新、DAG verify 全自动。含交互式 HITL（候选选择/模糊澄清/风险确认）+ `--non-interactive`（CI 用）。66 个新测试，合计 2908 PASS，SKIP=0。参见 [CHANGELOG](CHANGELOG.md)。
...
- **v2.17.0 (规划中)** — PHENOMENON 的 impl/test 自动波及完成 (AC #8)、Codex wrapper。
