...
- **v2.14.0 (当前)** — 结构缺陷 8 件一次修复 (cmd_466 dogfood)。sidecar `<test>.codd.yaml` 的 `verified_by:` (C6) / `axis_matrix:` (C9)、lexicon schema SSoT、completeness_audit 批量、scan.exclude bug fix (amber -52%)、`codd dag verify --auto-repair`、elicit mock-AI sentinel、AI timeout 3600s SSoT。dogfood 重跑：red 22 → 0。参见 [CHANGELOG](CHANGELOG.md)。
- **v2.13.0** — opt-out 保护：`OptOutPolicy` 要求所有配置级 opt-out 必须提供 `justification` + `expires_at`。废除静默 SKIP，保留 severity。`codd validate` 报告策略违规。
...
- **v2.15.0 (规划中)** — C8 `ci_health` 运行时模式（opt-in `ci.runtime_check: true`）。
