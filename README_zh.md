...
| 🧪 **`codd verify --runtime`** | Step 8 运行时 smoke 网关 (v2.19.0+)。DB 启动 + dev server 可达 + smoke HTTP + 真实浏览器 E2E (对运行中服务器)。`--runtime-skip {db,dev-server,connectivity,e2e,verification-test}` 按类别跳过并将原因记录到 report。 |
| 🔁 **`codd skills {install,list,remove}`** | 将内置 skill (例 `codd-evolve`) 分发至 `~/.claude/skills/` 与 `~/.agents/skills/`。支持 `--target {claude,codex,both}` / `--mode {symlink,copy}`，幂等、`--force` 时备份后覆盖。 |
| 🪡 **codd-evolve skill** | Brownfield 对话式演进。从自然语言意图一次走完 requirements → design → lexicon → source → tests → verify → propagate → Step 8 runtime smoke。内置 6 种 stop-and-ask 网关 (新词条 / breaking change / coherence violation / scope explosion / 角色范围 / 1:N UI 拓扑) 及 orchestrator 驱动的事前批准 branch。 |
| ⚡ **Codex App Server 后端** (v2.20.0) | `codd.yaml` 中 `codex_app_server.enabled: true` 时，将 AI 调用切换到持久 JSON-RPC 线程 (替代 subprocess)。`thread_strategy: per_session` 让 `codd implement` / `codd verify --auto-repair` / `codd fix` 共享一根线程以摊销 codex 冷启动。当二进制或 socket 缺失时自动 subprocess 回退。 |
...
- **v2.20.0 (当前)** — **Codex App Server JSON-RPC 集成** (cmd_357)。`AiCommand` Protocol + `CodexAppServerAiCommand` + `ai_command_factory.get_ai_command()` 将 `codd implement` / `codd verify --auto-repair` / `codd fix` 经由持久 JSON-RPC 会话调用 (`codex_app_server.enabled: true`)。默认仍为 subprocess 以保证完全向后兼容；当 codex 二进制缺失、unix socket 不可达或 `thread.start` 报错时自动 fallback（含 WARNING 日志）。`thread_strategy: per_session` 在多轮 turn 间复用同一线程以摊销冷启动开销。合计 3013 PASS、SKIP=0。
- **v2.19.0** — **CoDD 全面 OSS 化** (cmd_333)。废除 legacy `codd-pro` Pro Gate；`codd verify` 直接调用 `propagator.run_verify`。删除 `codd review` / `codd audit` / `codd risk` (无实际实现，受影响用户 0)。`bridge.py` 保留作为未来 verify 插件接口。另外 5 项横向特性：
  - `codd verify --runtime` (cmd_338) — Step 8 *运行时 smoke 门控* (DB 启动 + dev server 可达 + smoke HTTP + 真实浏览器 E2E) 成为最终完成检查；`--runtime-skip {db,dev-server,connectivity,e2e,verification-test}` (cmd_342) 按类别跳过并把原因写入 report。
  - `verify.verification_timeout.{per_node_seconds,total_seconds}` (cmd_342) 限制单个 verification_test 与总 verify 预算；超出的剩余节点记录为 `skipped: total_timeout_exceeded` 而非 red。
  - `ai_commands.impl_step_derive` 配置 + `warn_if_operation_flow_unused()` (cmd_345) — 当项目声明 `operation_flow` 但缺少对应 AI 命令时输出 WARNING，避免 `operation_flow_hint` 静默跳过被忽视。
  - `operation_flow:` YAML frontmatter (cmd_340) — 在 requirements 中按操作声明 actor / verb / target / parent / ui_pattern；`operation_flow_hint()` 经由 `criteria_expander.py` / `impl_step_deriver.py` 将拓扑注入 LLM 提示。
  - `codd dag verify` 增加 `ui_coherence_for_one_to_many` (cmd_340 B) — 当 1:N 关系缺少 master-detail / drilldown UI 证据时给出 amber 警告。
- **codd skills CLI** (cmd_336) — `codd skills {install,list,remove}` 将内置 skill (例 `codd-evolve`) 分发至 `~/.claude/skills/` 与 `~/.agents/skills/`，支持 `--target {claude,codex,both}` / `--mode {symlink,copy}`，幂等、`--force` 触发备份后覆盖。
- **codd-evolve skill** (skills/codd-evolve/) — Brownfield 对话式演进。从自然语言意图一次跑完 requirements → design → lexicon → source → tests → verify → propagate → Step 8 runtime smoke。内置 6 种 stop-and-ask 门 (new lexicon term / breaking change / coherence violation / scope explosion / 角色范围模糊 / 1:N 数据模型 UI 拓扑)，并提供 orchestrator 驱动场景的事前批准 branch。
- **v2.18.0** — 来自 [@v-kato](https://github.com/v-kato) 的 Greenfield triage (cmd_473)。Issue #20: `codd implement run --language` 可按调用粒度覆盖 `project.language` (避免 `codd init --language` 选错时的全 Wave 重生成 1h+ 成本)。Issue #21: 将 `detailed_design` 加入 `DEFAULT_NODE_PREFIXES`，消除 `codd plan --init` 生成的 node_id 被自家 `codd validate` 拒绝的自相矛盾。Issue #22: `_strip_code_fence` 改为非贪婪 + 不锚定行尾，截掉 LLM 在闭合 fence 之后追加的 markdown 解说。新增 12 项测试，合计 2937 PASS，SKIP=0。
...
- **规划中** — PHENOMENON 的 impl/test 自动波及完成 (AC #8)、App Server 基准测试公开、lexicon 插件市场。
