...
| 🧭 **`codd operations {derive,show,approve,merge}`** | 检测未挂接到任何已声明 operation 的需求单元，由 AI 提出 `operation_flow` 条目，并写出供人工审阅的 proposal —— 在 `approve` + `merge` 之前不会向 `codd.yaml` 写入任何东西。opt-in、非破坏性。 |
| 📜 **`codd contract {show,suggest,adopt,verify}`** | V 模型 artifact contract。`suggest`/`adopt` 以需求驱动的方式选择「项目在每个 stage *声明哪些* 目录 artifact」（`suggest` 检测项目的信号 —— 需求文档、设计文档、lexicon、已声明的 `operation_flow`、源码目录、test/e2e 套件 —— 并写出一份可审阅的提案，绝不触碰 `codd.yaml`；`adopt` 将其非破坏性且幂等地合并，`--enable` 用于开启该 gate）。`verify`/gate 是完成侧: 启用 contract 后，每个 stage 在其声明的 artifact 产出/有效之前无法宣称完成。`show` 渲染目录 + 本项目的 contract。opt-in，contract 缺失/禁用则为 no-op。 |
