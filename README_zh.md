...
- **v2.17.0 (当前)** — `node_completeness` 支持 `kind: common` (cmd_470)。修复 v2.15.0 引入的疏漏：当 `expects` 边指向 common（共享基础设施）节点时，即使实体文件存在，`node_completeness` 仍会误报为 missing。新增 6 项测试，合计 2914 PASS，SKIP=0。参见 [CHANGELOG](CHANGELOG.md)。
