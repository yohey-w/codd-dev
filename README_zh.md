...
- **v2.15.0 (当前)** — 引入 `kind: common` (cmd_467)。通过 `common_node_patterns:` glob 或 frontmatter 将共享基础设施（DB 客户端、middleware、配置文件等）分类为 common。C5 amber −79.2%（125 → 26）。`**` glob translator 修复。参见 [CHANGELOG](CHANGELOG.md)。
...
- **v2.16.0 (规划中)** — `codd fix [PHENOMENON]` — 用自然语言描述想修复的现象，CoDD 自动定位相关设计文档并更新、更新实现与测试、DAG verify 全自动。含交互式确认模式。
