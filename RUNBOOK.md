...
`codd init` はプロジェクト直下に `.prettierignore` と `.gitignore` を**無ければ**置く
（既にあれば触らない）。この2つは以下の2事故を防ぐためのもので、消すなら代替を用意すること。
さらに `<codd-dir>/canon.lock`（正本ハッシュ台帳）を生成する——**これはコミットする**。

**既存プロジェクトへ後から入れる場合**（`codd init` を通っていないリポ）は、
`codd canon accept` を1回叩いて `canon.lock` をコミットすれば同じ守りが効く。

#### ⚠️ 事故1: 整形ツールがCoDD正本を壊す

要件unitは**Markdownテーブルの1行**である。整形ツール（prettier等）は
**意味を変えずに見た目だけ**変えるので、テーブルの桁揃えが入っても
レビューで気づけず、チェックも全部緑のまま通る。それでも
「人間が承認した原本」とのバイト同一性は壊れている。

実測: `npx prettier --write .` が要件定義書のテーブルを**440行ぶん**書き換えた。
差分検証を別の理由でたまたま流さなければ、黙って壊れたままだった。

⇒ **`docs/requirements/` と `codd/` は、いかなる整形ツールの対象にもするな。**
`.prettierignore` はその宣言であり、他の整形ツール（dprint, markdownlint --fix,
IDEのformat on save）を使うなら**そちらにも同じ除外を設定すること**。

##### 第2層: 正本ハッシュ台帳（`codd canon`・道具に依存しない）

`.prettierignore` は**prettierという1つの道具**を止めるだけで、しかも
**新規プロジェクトにしか置かれない**。dprintにも、`markdownlint --fix` にも、
エディタのformat on saveにも、うっかりの一括 `sed` にも効かない。

そこで CoDD は**承認した時点のバイト列そのもの**を記録する:

```bash
codd canon status                    # 正本の範囲と、台帳と一致しているかを表示
codd canon accept --for cmd_042      # いまの正本を台帳に記録する（作業参照は必須）
```

`canon.lock` は**コミットする**。以後 `codd dag verify` の `canon_integrity` と
pre-commitフックが毎回照合し、**台帳と実ファイルが1バイトでも違えば赤で落ちる**。
何が書き換えたか（prettierか、dprintか、AIか、人間か）は問わない。

**🔴 これは認可ゲートではない。守れる範囲を正確に言う。**

| | |
|---|---|
| **捕まえる** | **無自覚な変更**——整形ツール・format on save・一括sed・エージェントの誤爆・雑なマージ。**誰も気づいていないので、誰もacceptしない**から検出できる |
| **捕まえない** | **意図的な変更＋accept**。編集した本人がacceptすれば緑になる。これは仕様であり、設定で変えられない |

⇒ 悪意ある改変への防御は、台帳に記録される**作業参照**と**コードレビュー**であって、
このチェックではない。「CoDDが正本を守っている」と**過大に言わないこと**。

**台帳は自動更新しない。** 勝手に更新される台帳は何も検出しない。書き換えるのは
`codd canon accept` だけであり、それは**差分を見たうえでの人間の行為**である。
`accept` は反射で叩けないよう作ってある:

1. **`--for <作業参照>` が必須**。無しで叩くと exit 2 で拒否され、台帳は動かない
2. **何を承認しようとしているかの差分を必ず表示する**（`git diff --stat` ＋ 実パッチ）
3. その作業参照は**台帳の各行に書き込まれてコミットされる**ので、中身の無いacceptは
   `git log` / `git blame` に**永久に残る**（＝レビューの手がかり。認可ではない）

要件を意図的に変えたときの正規の手順:

```bash
git diff -- docs/requirements/            # まず差分を読む
codd canon accept --for cmd_042           # 意図した変更なら台帳を更新
git add docs/requirements/ codd/canon.lock && git commit   # 台帳を変更と一緒にコミット
```

意図しない変更だった場合は `git checkout -- <path>` で戻す。

###### 🔴 エージェントへの規律（accept の形骸化を防ぐ）

> **acceptしてよいのは、その文書を変えること自体が自分のcmdの目的だった場合のみ。**
> **検査を通すためのacceptは禁止。**
>
> `canon_integrity` が赤になったとき、自分のタスクが「要件書を変える」ことでは
> なかったなら、それは**自分が壊した（か、道具が壊した）証拠**である。
> `codd canon accept` ではなく `git checkout -- <path>` が正しい手。
> 赤を緑にするためだけの accept は、この機構を完全に無効化する唯一の使い方であり、
> **台帳に作業参照が残るので後から必ず露見する**。

同じ文言は `codd canon accept` の出力・`canon_integrity` の失敗メッセージ・
pre-commitフックの拒否メッセージにも入れてあるので、赤を見たエージェントは
「acceptすれば緑になる」だけを学習することはない。

###### 台帳フォーマットとマージ

1文書=1行（`<path>\t<sha256>\t<作業参照>`）・パス順・**行間に空行**。
JSONの入れ子をやめたのは、**別々の文書をacceptした2ブランチが衝突なくマージできる**
ようにするため（空行はgitの変更領域が隣接して衝突するのを防ぐ実測に基づく措置）。
**同じ文書を両ブランチでacceptした場合は衝突する**——それは本当の不一致なので正しい。

| 状態 | 判定 | 理由 |
|---|---|---|
| 台帳にある文書が**変わった/消えた** | 🔴 赤（`canon.severity`、既定 `red`） | 承認済み文書が正規の経路外で変わった＝これが防ぎたい事故そのもの |
| 範囲内なのに**台帳に無い**（新規追加） | 🟡 黄 | ファイルの追加は `git status` で見える。赤にすると要件書を最初に書いた瞬間に落ちる |
| **台帳そのものが無い**（既存プロジェクト） | 🟡 黄＋案内 | CoDDを上げただけで既存リポが壊れてはならない。`codd canon accept` の案内だけ出す |
| 範囲内に文書が無く台帳も無い | ⚪ SKIP | まだ守るものが無い（本当に休眠） |

**正本の範囲**は既定で**要件文書**（`requirement_reconciliation` と同じ発見経路＝
`docs/requirements/**/*.md` ＋慣例的なトップレベルファイル。`requirement_reconciliation.docs`
を明示していればそれ）。`scan.exclude` は必ず効くので、`docs/source/**` のような
**参照素材は正本ではない**。範囲を広げたいときは `codd.yaml` の `canon.docs` に列挙する。

⚠️ **既定を「`doc_dirs` 配下の全 `*.md`」にしていないのは意図的**である。
`codd generate` / `propagate` / `fixup-drift` / `extract` は設計書・テスト設計書を
**正当に書き換える**。それらを台帳に入れると**通常運転のたびに赤**になり、利用者は
`accept` を反射で叩くようになる——**警報疲れで死んだ検出器**は、無い検出器より悪い。

⚠️ **`dag.enabled_checks` を固定しているプロジェクトは `canon_integrity` を足すこと。**
足さないとこのチェックは**黙って走らない**（`codd dag verify` が未選択チェックの
注意書きを出す）。同梱の project_type プロファイル（web/cli/mobile/iot）は更新済み。

#### ⚠️ 事故2: `docs/requirements/` に写しを置くと二重計上される

`docs/requirements/**/*.md` は **rglob で全件**が要件文書として発見される。
参照用に元の要件書の写しを `docs/requirements/original/` へ置いた瞬間、
**要件unitが2倍に化ける。警告は出ない**——しかも増えた数が正しいのか
間違っているのかを判定する材料が利用者側に無い。

⇒ **`docs/requirements/` は正本だけ**。参照素材・受領した元文書・過去版は
`docs/source/` など**外**に置く。どうしても中に置くなら
`codd.yaml` の `requirement_reconciliation.docs` に正本を明示列挙して発見を上書きする。
...
全ドキュメントのフロントマターを読み取り、依存グラフ（`codd/scan/`）を再構築する。
- 自動生成データは毎回リフレッシュ
...
#### ⚠️ ローカル緑は、デプロイ先の緑を証明しない

**「どの環境で緑になったか」を成果物に必ず刻め。** 「ローカルで緑」と
「デプロイ先で緑」は**別の主張**であり、納品の根拠になるのは後者だけである。

実測例: 使い捨てのローカルPostgreSQLを立ててRLSと公開関数をend-to-endで検証し
全部緑にした。それでも本番では全ユーザーで結果が0件になった。原因は
拡張機能のインストール先スキーマの差（ローカルは `public`、マネージド基盤は
`extensions`）で、`security definer` 関数の `search_path` を固定するという
**それ自体は正しい防御**が、本番でだけ関数を見えなくしていた。

この型が危険なのは**失敗が「0行を返す」という形で出る**こと——画面は
「データがありません」になるだけで、**成功と見分けがつかない**。

⇒ **リリース条件に「デプロイ先での実検証1回」を必ず含める。**
検証はローカル環境固有の前提（DB拡張の配置・環境変数・ネットワーク・
権限モデル）に依存しない形で書き、**再現可能なスクリプトとしてリポジトリに残す**
（例: `infra/verify-rls.sql`）。マネージド基盤（Supabase / RDS / Cloud SQL 等）は
素のエンジンと構成が異なる前提で扱うこと。
...
- `codd/scan/` は `.gitignore` に入れる（スキャンで再生成可能）
