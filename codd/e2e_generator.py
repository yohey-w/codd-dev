...
def _scenario_from_block(name: str, block: str) -> UserScenario:
    ...
    kind = _extract_field(block, "Kind") or "user_journey"
    actor = _extract_field(block, "Actor")
    coverage_axis = _extract_field(block, "Coverage Axis")
    source_operation = _extract_field(block, "Source Operation")
    source, operation_id = _parse_source_operation(source_operation)
    trigger = _extract_field(block, "Trigger")
    routes = _parse_routes(_extract_field(block, "Routes") or "")
    preconditions = [
        item
        for item in _extract_markdown_list_section(block, "Preconditions")
        if item != "No explicit preconditions declared."
    ]
    steps = _extract_markdown_list_section(block, "Steps")
    observable_outcomes = [
        item
        for item in _extract_markdown_list_section(block, "Observable Outcomes")
        if item != "No explicit observable outcomes declared."
    ]
    acceptance = [
        item
    ...
    return UserScenario(
        ...
        kind=kind,
        actor=actor if actor and actor != "unspecified" else None,
        coverage_axis=coverage_axis if coverage_axis and coverage_axis != "unspecified" else None,
...
def _parse_source_operation(value: str | None) -> tuple[str | None, str | None]:
    if not value or value == "unknown#unknown":
        return None, None
    if "#" not in value:
        return value, None
    source, operation_id = value.rsplit("#", 1)
    ...
    if scenario.kind == "operational":
        ...
        lines.extend(
            [
                ...
                f"// codd: covers operation={_clean_comment(source_operation)} axis={_clean_comment(coverage_axis)}",
                f"// Actor: {_clean_comment(scenario.actor or 'unspecified')}",
                f"// Coverage axis: {_clean_comment(coverage_axis)}",
                f"// Source operation: {_clean_comment(source_operation)}",
...
def _source_operation_label(scenario: UserScenario) -> str:
