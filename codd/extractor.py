...
def synth_docs(facts: ProjectFacts, output_dir: Path) -> list[Path]:
    ...
    # Generate system-context.md
    ctx_path = output_dir / "system-context.md"
    ctx_path.write_text(_render_system_context(facts, today), encoding="utf-8")
    generated.append(ctx_path)

    # Generate per-module docs
    for mod_name, mod in sorted(facts.modules.items()):
        if not mod.files:
            continue
        slug = re.sub(r'[^a-z0-9]+', '-', mod_name.lower()).strip('-')
        mod_path = modules_dir / f"{slug}.md"
        mod_path.write_text(
            _render_module_doc(mod, facts, today),
            encoding="utf-8",
        )
...
def _render_system_context(facts: ProjectFacts, today: str) -> str:
    """Render system-context.md."""
    # Build depends_on for frontmatter (empty at system level)
    lines = [
        "---",
        "codd:",
        ...
        "",
        "> Auto-generated by `codd extract`. Review and promote to `codd/` when confirmed.",
        "",
    ]

    # Language & frameworks
    lines.append(f"**Language**: {facts.language}")
    if facts.detected_frameworks:
        lines.append(f"**Frameworks**: {', '.join(facts.detected_frameworks)}")
    if facts.detected_orm:
        lines.append(f"**ORM**: {facts.detected_orm}")
    if facts.detected_test_framework:
        lines.append(f"**Tests**: {facts.detected_test_framework}")
    lines.append(f"**Source**: {', '.join(facts.source_dirs)}")
    lines.append(f"**Total**: {facts.total_files} files, {facts.total_lines:,} lines, {len(facts.modules)} modules")
    lines.append("")

    # Module map table
    lines.append("## Module Map")
    lines.append("")
    lines.append("| Module | Files | Lines | Classes | Functions | Dependencies | Patterns |")
    lines.append("|--------|-------|-------|---------|-----------|-------------|----------|")

    for mod_name, mod in sorted(facts.modules.items()):
        classes = sum(1 for s in mod.symbols if s.kind == "class")
        funcs = sum(1 for s in mod.symbols if s.kind == "function")
        deps = ", ".join(sorted(mod.internal_imports.keys())) or "—"
        patterns = ", ".join(mod.patterns.values()) or "—"
        lines.append(
            f"| {mod_name} | {len(mod.files)} | {mod.line_count:,} | "
            f"{classes} | {funcs} | {deps} | {patterns} |"
        )
    ...
    has_deps = False
    for mod_name, mod in sorted(facts.modules.items()):
        for dep in sorted(mod.internal_imports.keys()):
            if dep in facts.modules:
                lines.append(f"{mod_name} → {dep}")
                has_deps = True
    if not has_deps:
    ...
    for mod in facts.modules.values():
        all_external.update(mod.external_imports)
    if all_external:
...
def _render_module_doc(mod: ModuleInfo, facts: ProjectFacts, today: str) -> str:
    """Render a per-module design document."""
    slug = re.sub(r'[^a-z0-9]+', '-', mod.name.lower()).strip('-')
    node_id = f"design:extract:{slug}"

    # Build depends_on
    depends_on_entries = []
    for dep_name in sorted(mod.internal_imports.keys()):
        if dep_name in facts.modules:
            dep_slug = re.sub(r'[^a-z0-9]+', '-', dep_name.lower()).strip('-')
            depends_on_entries.append(
                f'    - id: "design:extract:{dep_slug}"\n      relation: imports'
            )
    ...
    lines = [
        ...
        "  type: design",
        "  source: extracted",
        f"  confidence: {_module_confidence(mod):.2f}",
        f"  last_extracted: \"{today}\"",
    ]
    if depends_on_entries:
        lines.append("  depends_on:")
        lines.extend(depends_on_entries)
    lines.extend([
        "---",
        "",
        f"# {mod.name}",
        "",
        f"> {len(mod.files)} files, {mod.line_count:,} lines",
        "",
    ])

    # Patterns
    if mod.patterns:
        lines.append("**Role**: " + ", ".join(mod.patterns.values()))
        lines.append("")
    ...
    if funcs:
        ...
        lines.append("")
        for sym in funcs[:30]:  # Cap at 30 for readability
            if sym.params:
                lines.append(f"- `{sym.name}({sym.params})` ({sym.file}:{sym.line})")
            else:
                lines.append(f"- `{sym.name}()` ({sym.file}:{sym.line})")
        if len(funcs) > 30:
            lines.append(f"- ... and {len(funcs) - 30} more")
        lines.append("")

    # Internal dependencies
    if mod.internal_imports:
        lines.append("## Internal Dependencies")
        lines.append("")
        for dep_name, imp_lines in sorted(mod.internal_imports.items()):
            lines.append(f"### → {dep_name}")
            for imp in imp_lines[:5]:
                lines.append(f"- `{imp}`")
            if len(imp_lines) > 5:
                lines.append(f"- ... and {len(imp_lines) - 5} more")
        lines.append("")
    ...
    if mod.external_imports:
        lines.append("## External Dependencies")
        lines.append("")
        for dep in sorted(mod.external_imports):
            lines.append(f"- `{dep}`")
        lines.append("")
    ...
    if mod.test_files:
        lines.append("## Tests")
        lines.append("")
        for t in sorted(mod.test_files):
            lines.append(f"- `{t}`")
        lines.append("")
...
def _module_confidence(mod: ModuleInfo) -> float:
    """Calculate confidence score for a module's extracted data."""
    score = 0.5  # Base: we found files

    # More files = more evidence
    if len(mod.files) >= 3:
        score += 0.1
    # Has symbols = we could parse it
    if mod.symbols:
        score += 0.1
    # Has tests = verification exists
    if mod.test_files:
        score += 0.1
    # Has patterns detected = stronger structural understanding
    if mod.patterns:
        score += 0.05

    return min(score, 0.85)  # Cap below green band — human review needed
