...
_TEST_DOC_CANONICAL_VB_HEAD = [
    ...
    "- A VB id is IMMUTABLE once issued: never renumber, rename, or repurpose an existing id in a later revision. Downstream test markers (`codd: covers vb=<id>`) bind to the id, so changing it silently orphans every test that covered it. To retire a behavior, remove its row; to add one, mint a fresh id.",
    "- Express each VB row as an OBSERVABLE input→consequence at the system's PUBLIC surface: what a caller/user does (input, invocation, event) and the consequence they can observe (return value, raised error, emitted output, produced artifact, persisted-then-read-back state). A property that can only be checked by reaching into private/internal state with no public-surface consequence is NOT a verifiable behavior — do not give it a VB row; capture it in the design prose instead. This keeps every declared id provable by a test that runs the system and asserts on what it observes, never by an assertion disconnected from execution.",
