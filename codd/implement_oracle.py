...
def certify_oracle_scope(
    ...
    CONTRACT PATH (Contract Kernel oracle dispatch §5): a contract-path language
    (modeled oracle + registered adapter — today Go via ``go-toolchain``) certifies
    through its adapter's ``certify_scope`` (which raises ``OracleScopeError`` on a
    missing go.mod / undecidable module path / empty module). Generic predicate —
    no hardcoded language-name comparison here.
    """
...
def _run_contract_oracle(
    ...
    * ``adapter`` → the registered adapter's ``execute`` (an in-process composite;
      a declared ``kind="adapter"`` with no ``execute`` is an incomplete contract →
...
def _run_oracle_command(
    ...
    CONTRACT PATH (Contract Kernel oracle dispatch §3–§5): a language whose
    resolved ``LanguageProfile`` declares a modeled ``implement_oracle`` with a
    REGISTERED adapter runs on the generic command-sequence executor instead of a
    hand-written executor. Today only Go (``go-toolchain`` registered) qualifies, so
    Go's ``kind="composite"`` spec routes here; Python is handled by its own branch
    above (and its adapter is not registered), and TS (``kind="compiler"``) keeps
    the legacy ``tsc`` path below. The predicate is GENERIC (modeled oracle +
    registered adapter via :func:`_resolve_contract_oracle`) — there is no
    hardcoded language-name comparison in this dispatch.
    """
    if profile.language == "python" and spec.kind == "composite":
    ...
    if spec.kind == "composite":
        ...
        if contract is not None:
            lang_profile, oracle_decl, adapter = contract
            return _run_contract_oracle(project_root, lang_profile, oracle_decl, adapter, config)
