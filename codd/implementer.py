    ...
    if _spec_targets_tests(spec, config):
        ...
        lines.extend(
            [
                ...
                "- Bind an assertion to an exact surface form — a verbatim message substring, exact output bytes, an exact ordering or layout — ONLY when the design or its dependency documents pin that surface form; when they pin it, assert it verbatim and do not weaken it, because a deviation from a pinned form is a real defect the test must keep RED.",
                "- When the documents pin an OUTCOME but not its surface form (an error condition, a computed value, a status), assert the semantic property the documents actually pin: the error/exception type and that it signals the designed condition, the parsed or typed value rather than its rendered string, the presence of the designed fields — never incidental wording, token spelling, whitespace, or formatting the documents left free. An unpinned surface form is implementation freedom; binding a test to it turns a correct implementation that merely words its result differently into a false failure.",
                "- Precision, not weakening: choosing a pinned property over an unpinned surface form must never soften the check — every assertion must still FAIL when the designed behavior is broken.",
                "- The same pinnedness rule governs the SHAPE of a produced value — the names of its properties or fields, whether a given key is present, the discriminator/tag string that selects one variant of a result — even when that shape is invisible to a type system. When you assert such a shape through means a typechecker cannot see (string-keyed lookups into a returned value, membership/`in`-style key-presence checks, property-name matcher assertions, reflection or dictionary lookups), every property name, key, or tag string you assert must be verbatim-traceable to the design's pinned surface OR to a declaration in the injected dependency-produced files below. A shape the documents never pinned is implementation freedom — assert the semantic outcome the documents DO pin (the resulting behavior, the typed value, the designed condition), not a guessed field layout.",
                "- Where the project provides a type system, PREFER importing the producer's declared types and asserting the produced value through typed access, so the compiler proves the shape binding instead of a stringly-typed probe that can drift silently. Reserve type-invisible key/tag assertions for shapes the documents pin that no producer exports as a checkable type. This is the same non-weakening rule as the pinned-surface case: a type-invisible shape assertion must still FAIL when the designed shape is wrong.",
                "",
                ...
                "",
                "Structural / architecture governance tests are DESIGN-GATED (do not invent them):",
                "- A generated test must NOT read, glob, or parse the project's OWN source tree, nor assert its internal module/layer/dependency structure (which internal unit may depend on which, package/boundary direction, an allowed-dependency map), UNLESS the design documents EXPLICITLY declare such a governance/architecture test as a required behavior.",
                "- WHEN the design does declare one, derive the allowed-dependency / structure data VERBATIM from the dependencies the design itself declares — do NOT hand-author a layer map or an allowed-imports table inside the test. An invented copy of the dependency contract is a second source of truth that can itself be wrong; a declared governance test READS the declared structure, it does not restate it.",
                "- Absent such a declaration, do not generate a structural gate at all: assert the system's designed BEHAVIOR, not the shape of its source tree. The project's own toolchain owns its architecture, and a self-authored structural probe only adds a drift axis.",
                "",
            ]
        )
