"""POSIX shell script launcher for the CDP browser cookbook."""
...
@register_cdp_launcher("shell_script")
class ShellScriptLauncher(CdpLauncher):
    """Launch a CDP-enabled browser through a project-owned shell script."""

    launcher_name: ClassVar[str] = "shell_script"
    ...
    def launch_command(self, browser_config: Mapping[str, Any]) -> list[str]:
        ...
        script_path = _env_required(
            browser_config,
            key="script_path_env",
            default_env="CODD_CDP_SHELL_SCRIPT",
        )
        return [script_path, *_string_list(browser_config.get("args", []))]

    def teardown_command(self) -> list[str]:
        explicit = _string_list(self._last_config.get("teardown_command", []))
        if explicit:
            return explicit
        teardown_script = os.environ.get(
            str(self._last_config.get("teardown_script_env") or "CODD_CDP_TEARDOWN_SCRIPT")
        )
        return [teardown_script] if teardown_script else []
