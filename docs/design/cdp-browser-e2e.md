...
しかし、cmd_393 v1.25.0 release 後、example-app の HTTP × `__Secure-` Cookie 事故修復確認は **将軍が手動で CDP-Edge を 9224 で起動して E2E を回す** という属人的フローで行われた。
...
3. navigate http://0.0.0.0:3000/login
...
- example-app 実証 (今夜の手動フローを CdpBrowser で再現)
...
```yaml
# example-app/docs/design/auth_design.md frontmatter
user_journeys:
...
| `example-app` / 他 project 名 | project の codd.yaml + journey 宣言 |
...
```bash
grep -rEi 'powershell|msedge|nextauth|__secure-|react|native[_-]setter|example-app|wsl|windows' \
  codd/deployment/providers/verification/cdp_browser.py \
...
1. `example-app/codd.yaml` に `verification.templates.cdp_browser` 設定を追加
2. cookbook から `powershell_script` launcher / `edge` engine / `react_native_setter` strategy を `example-app/codd_plugins/cdp_browser/` にコピー
...
| 2026-05-06 早朝 | example-app 修復確認は将軍が手動 CDP-Edge 9224 で実行 — 自動化されていなかった |
...
| 設計骨子 YAML | `orchestrator:queue/reports/gunshi_397_g0_design.yaml` |
