...
Do NOT use for:
- Modifying applications built with CoDD (example-app etc.) → use `/codd-evolve`
...
```bash
# 1a. CoDD DAG 状態確認 (dogfood project)
cd /home/tono/example-app   # or whichever CoDD project you use
codd dag verify 2>&1 | tail -5
# → red=0 であること。red > 0 なら先に red を解消してから修正着手

# 1b. 現状テスト (codd-dev)
cd /home/tono/codd-dev
...
- domain agnostic か (example-app 固有でないか)
...
```
□ この変更は example-app 以外のプロジェクトでも役立つか?
...
□ 第三者 (example-app を知らない開発者) がこの変更で恩恵を受けるか?
```
...
- **example-app 固有のノード名・テーブル名を CoDD core にハードコードしない**
...
変更後、必ず実 CoDD プロジェクトで動作確認する。example-app を標準的な dogfood 環境として使う。
...
# 6d. 変更が affect した check を example-app で実際に確認
...
**儂が勝手に v3.0.0 を切ることは禁止**。v3 はブランド戦略で決まる (殿確定 2026-05-10)。
...
```bash
bash /path/to/orchestrator/scripts/inbox_write.sh karo \
  "CoDD修正完了。[変更種別]. pytest PASS/SKIP=0. dogfood codd dag verify red=0. commit [hash]." \
  report_received gunshi
```
...
| F2 | example-app 固有名 (delivery_target / course_master 等) を CoDD core にハードコード | LMS に overfit = 汎用性破壊 |
...
| F10 | 儂が具体的な CoDD 修正案を出す (Shogun が修正方針を具体提案する) | Shogun が提案すると LMS overfit する。具体事故は渡す、修正案は軍師が抽象化 |
...
example-app での推奨コマンド: `codd verify --runtime --runtime-skip verification-test`
...
- `codd verify --runtime`: dev server + DB 起動状態が必要。
  ...
  example-app では `codd.yaml` に `verify.verification_timeout` 設定済み。

---

## Configuration Reference (example-app 実例)
...
verify:
  verification_timeout:
    per_node_seconds: 30    # WSL2 dev env 向け
    total_seconds: 120      # 4 CDP nodes × 30s
```
