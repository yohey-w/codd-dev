...
def _config() -> dict:
    return {
        ...
        "resource_group": "rg-example-app-prod",
...
def test_app_service_target_init(monkeypatch):
    ...
    assert target.resource_group == "rg-example-app-prod"
    assert target.app_name == "example-app-prod"
...
def test_snapshot_calls_az_webapp_show(monkeypatch):
    ...
    def fake_run(command, capture_output, text, check):
        ...
        return _completed('{"defaultHostName": "example-app-prod.azurewebsites.net"}')
    ...
    assert "rg-example-app-prod" in calls[0]
    assert "--name" in calls[0]
    assert "example-app-prod" in calls[0]
...
def test_snapshot_returns_info(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: _completed(
            '{"defaultHostName": "example-app-prod.azurewebsites.net", "state": "Running"}'
        ),
    )

    snapshot = AppServiceTarget(_config()).snapshot()

    assert snapshot["app_name"] == "example-app-prod"
    assert snapshot["resource_group"] == "rg-example-app-prod"
    assert snapshot["default_host_name"] == "example-app-prod.azurewebsites.net"
...
def test_healthcheck_delegates_to_deployer(monkeypatch):
    config = _config()
    config["healthcheck"] = {
        "url": "https://example-app-prod.azurewebsites.net/api/health",
        "expected_status": 204,
        "timeout_seconds": 12,
        "retries": 4,
    }
    ...
    assert calls == [("https://example-app-prod.azurewebsites.net/api/health", 204, 12, 4)]
