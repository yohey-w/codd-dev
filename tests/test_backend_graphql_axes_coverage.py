"""Coverage-axis tests for the backend_graphql lexicon plug-in."""
...
def test_backend_graphql_axes_match_manifest_observation_dimensions():
...
def test_backend_graphql_axes_have_rationale_criticality_and_variants():
    axes = _yaml("lexicon.yaml")["coverage_axes"]
...
def test_backend_graphql_variant_ids_are_graphql_spec_literals():
    variant_ids = {
        variant["id"]
...
def test_backend_graphql_severity_rules_cover_each_axis_gap():
...
def test_backend_graphql_extension_contains_coverage_examples():
