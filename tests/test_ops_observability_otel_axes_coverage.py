"""Coverage-axis tests for the ops_observability_otel lexicon plug-in."""
...
def test_ops_observability_otel_axes_match_manifest_observation_dimensions():
...
def test_ops_observability_otel_axes_have_rationale_criticality_and_variants():
    axes = _yaml("lexicon.yaml")["coverage_axes"]
...
def test_ops_observability_otel_variant_ids_are_otel_literals():
    variant_ids = {
        variant["id"]
...
def test_ops_observability_otel_severity_rules_cover_each_axis_gap():
...
def test_ops_observability_otel_extension_contains_coverage_examples():
