"""Coverage-axis tests for the web_authn_webauthn lexicon plug-in."""
...
def test_web_authn_webauthn_axes_match_manifest_observation_dimensions():
...
def test_web_authn_webauthn_axes_have_rationale_criticality_and_variants():
    axes = _yaml("lexicon.yaml")["coverage_axes"]
...
def test_web_authn_webauthn_variant_ids_are_spec_literals():
    variant_ids = {
        variant["id"]
...
def test_web_authn_webauthn_severity_rules_cover_each_axis_gap():
...
def test_web_authn_webauthn_extension_contains_coverage_examples():
